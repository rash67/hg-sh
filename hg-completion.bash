...
_hg()
{
    #echo "_hg: [$1] [${COMP_WORDS[$COMP_CWORD]}] [${COMP_WORDS[$COMP_CWORD-1]}] [${COMP_WORDS[@]}] [$COMP_CWORD]\n"
...
_h-dash() {
  #echo "a:[$@] cw:[${COMP_WORDS[@]}] cwi:[$COMP_CWORD]\n"; 
  cmd=${COMP_WORDS[0]}
  
  if [[ $cmd = h-* ]]; then
    IFS=' ' read -a args <<< `alias $cmd | cut -f 2 -d= | tr -d \'`
    COMP_WORDS=(${args[@]} ${COMP_WORDS[@]:1} )
    #echo "#: ${#args[@]} [${args[@]}]"
    ((COMP_CWORD+=(${#args[@]} - 1)))              
    #echo "cw/cwi: [${COMP_WORDS[@]}] [$COMP_CWORD] [${args[0]}] [${COMP_WORDS[$COMP_CWORD]}] [${COMP_WORDS[$COMP_CWORD-1]}]\n"
    let x=$COMP_CWORD-1
    _hg "${args[0]}" "${COMP_WORDS[$COMP_CWORD]}" "${COMP_WORDS[$x]}"
  else 
    _hg $1 $2 $3
  fi
  return
...
install-complete() {
  complete -o bashdefault -o default -o nospace -F _h-dash $1 \
      || complete -o default -o nospace -F _h-dash $1   
}
...
for a in h-br h-co h-st h-s h-log h-lg h-back h-nc h-rb h-hei h-he h-rt \
         hg-br hg-co hg-st hg-s hg-log hg-lg hg-back hg-nc hg-rb hg-hei hg-he hg-rt \
       ; do
    install-complete $a;
done
